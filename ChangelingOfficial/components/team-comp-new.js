...
`
<section class="bigSegment">
        <div class="title">
            <img class ="paper" src="./images/paper-shadow.png">
            <h1 class="tText">{{whatTeam}}</h1>
        </div>
        <div class="next">
            <img src="./images/note.png" alt="Next" class="sticky">
            <p class="page-indicator">{{ activeRow }} / {{ teamRows }}</p>
            <a aria-controls="carousel" aria-label="next group" aria-role="button" class="carousel-control-next" :href="whatTeamHref" v-on:click="incrementRow" data-slide="next" id="nextButton">Next</a>
        </div>
        <div class="prev">
            <img src="./images/note.png" alt="Previous" class="sticky">
            <p class="turnText"><u> << Previous</u></p>
            <a aria-controls="carousel" aria-label="previous group" aria-role="button" class="carousel-control-prev" :href="whatTeamHref" v-on:click="decrementRow" data-slide="prev"><< Previous</a>
        </div>
		...
		<div :id="whatTeam" class="carousel slide carousel-multi-item" data-ride="carousel" data-interval="false">
			<div class="carousel-inner" role="carousel" aria-orientation="horizontal">
				<div class="carousel-item" v-for="n in teamRows">
        			<div class="row">
        			    <div v-for="teamMember in teamMembers.slice(0+(n*3-3), 3+(n*3-3))" class="container" tabindex="-1">
        			        <h5 class="infoh5">{{teamMember.name}}</h5>
        			        <p class="infop">{{teamMember.roles}}</p>
        			        <img  :id="teamMember.modalId" @click="teamMember.showModal = true" role="button" label="team member modal button" tabindex=0 v-on:keyup.enter="teamMember.showModal = true"  @error="imgOnError" class="frame" src="./images/polaroid_nb.png" alt="image of team member"> 
							<img class="person" :src="teamMember.img" alt="team member photo">
